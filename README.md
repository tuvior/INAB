...
INAB is a self-hosted importer for Swiss bank CAMT.053 exports into YNAB.
...
- Drag/drop CAMT.053 XML uploads.
...
CSV and MT940 exports are rejected in v1. See [docs/format-choice.md](docs/format-choice.md) for the rationale.
...
Open `http://127.0.0.1:8000`, log in, select your YNAB plan, then upload a CAMT.053 XML export once so the app can discover IBANs. Map each discovered IBAN to a YNAB account in Setup, then upload again to preview and import.
...
- Entries without a usable bank reference get a deterministic hash-based `INAB:<hash>` import ID.
